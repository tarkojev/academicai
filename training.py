"""
This file implements the student training script that distills knowledge from teacher models, which:
- supports two modes of operation: distillation from multiple teacher runs or from an already-stored ensemble.
- samples a few-shot support set from the training data (same as teachers) and gets soft targets.
- trains a student model on the support set using CE and KL Divergence loss.
- evaluates the trained student on the AG News test set.
- benchmarks the student in terms of parameter count, size, and inference latency.
- saves logits, probabilities, labels, and structured metadata for later analysis.
"""
...
# Helper functions for model building and prediction
def is_t5(name: str) -> bool:
    return name.startswith("t5-")

# Custom T5 encoder-classifier since T5 doesn't have a built-in classification head
...
# Build model and tokenizer based on name, with special handling for T5
...
# Configuration and metadata handling for teacher ensemble predictions
...
# Predict logits and labels for a dataset using a model and tokenizer
@torch.no_grad()
...
# Benchmark model configuration and inference latency
...
# Run multiple teacher trainings 
def run_teachers(args, device, train_ds, test_ds):
    ...
    for model_name in args.models:
        for seed in args.train_seeds:
            ...
            # Train teacher model on few-shot support set and evaluate on test set
            t0 = time.perf_counter()
            train_classifier(
                model=model,
                loader=loader,
            ...
            # Save this rin logits, probs, labels, and metadata
    ...
    # Save aggregated teacher results for this configuration
